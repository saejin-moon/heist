...
def test_evaluate_policies_expanded_metrics():
    ...
    expected_keys = [
        "win_rate", "mean_return", "mean_length", "mean_alarm",
        "scout_tag_rate", "terminal_rate", "loot_rate", "extraction_rate",
        "mean_hack_progress", "mean_explored_pct", "mean_tagged_pois", "mean_neutralized_guards",
        "cause_alarm_max_rate", "cause_guard_catch_rate", "cause_timeout_rate",
        "role_credit_scout", "role_credit_hacker", "role_credit_muscle", "role_credit_extractor"
    ]
    
    for key in expected_keys:
        assert key in metrics, f"Missing metric key: {key}"
        assert isinstance(metrics[key], (float, int, np.floating, np.integer)), f"Metric {key} must be numeric"
