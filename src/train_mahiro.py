"""
MAHIRO: Multi-Agent HIRO Baseline
Implements a two-timescale PPO where the Manager sets continuous goals (x,y).
The Worker receives an intrinsic reward based on L2 distance to the goal.
"""
...
def main():
    ...
    if prev_ckpt:
        ...
        load_matching_weights(agent, os.path.join(prev_ckpt, "mahiro.pt"), device)
    ...
    m_actions = torch.zeros((m_steps, N_AGENTS, args.num_envs, 2)).to(device)
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if step % args.macro_step == 0:
                ...
                with torch.no_grad():
                    ...
                    current_goals = action.view(N_AGENTS, args.num_envs, -1)
            ...
            # 2. Worker Step
            with torch.no_grad():
                g = current_goals.view(N_AGENTS * args.num_envs, -1)
        ...
        b_current_goals = m_actions.repeat_interleave(args.macro_step, dim=0).flatten(
            0, 2
        )
        ...
        if update % args.eval_every == 0 or update == num_updates:
            ...
            if args.save_model:
                ...
                torch.save(agent.state_dict(), f"checkpoints/{run_name}/mahiro.pt")

    if args.save_model:
        write_completion(run_name, "mahiro", args.total_timesteps, global_step)
