...
class ReplayBuffer:
    ...
    def push(self, obs, actions, rewards, dones, states, next_obs, next_states):
        for a in AGENTS:
        ...
        self.dones[self.pos] = float(dones)
...
def select_actions(env, obs, q_nets, epsilon, device):
    ...
    for a in AGENTS:
        ...
        with torch.no_grad():
            q = q_nets[a](obs_t, gs_t).squeeze(0)
        q = q.cpu().numpy()
...
def train(args: Args):
    ...
    while global_step < args.total_steps:
        ...
        while not done and global_step < args.total_steps:
            ...
            replay.push(obs, actions, rewards, done, states, next_obs, next_states)
            ...
            if replay.size > args.learning_starts and global_step % args.train_freq == 0:
                ...
                q_vals = []
                for a in AGENTS:
                    q = q_nets[a](batch["obs"][a], batch["gs"][a])
