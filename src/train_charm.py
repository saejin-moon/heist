...
def main():
    ...
    m_actions = torch.zeros((m_steps, N_AGENTS, args.num_envs, 2)).to(
        device
    )  # continuous goals
    ...
    if args.save_model:
        ...
        torch.save(agent.state_dict(), f"checkpoints/{run_name}/charm.pt")
        write_completion(run_name, "charm", args.total_timesteps, global_step)
