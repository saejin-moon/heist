...
7. **Curriculum Learning (`curriculum.py`):**
   ...
   * **Validated (scripted controller, `src/scripted.py` + `src/run_scripted_curriculum.py`):** every stage is solvable and difficulty ramps monotonically. A guard-avoiding BFS team with muscle neutralization and hacker door bypass wins 1.00 / 0.73 / 0.58 / 0.55 / 0.45 across stages 0-4 (40-episode revalidation, seed 100), with the causal chain completing 83-100% at every stage. Counterfactual analysis confirms scout/hacker/extractor are strictly necessary at all five stages (win -> 0 when no-op'd), so the RG-Dec-POMDP chain structure survives scaling. Full table in `results/README.md`.
